...
class TestAgentCustomizations:
    ...
    def test_student_agent_contract_structure(self):
        ...
        assert 'Use the `teacher` handoff when any of these conditions holds' in text
        assert 'Use the `engineer` handoff only to improve the structural clarity of your teacher-facing explanation' in text
        assert 'Implement the smallest defensible candidate revision' in text
        assert 'Treat turn-scoped `steering/<agent>/turn-N/STEERING.md` artifacts as authoritative for the current turn' in text
