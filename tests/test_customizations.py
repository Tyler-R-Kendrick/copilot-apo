...
class TestAgentCustomizations:
    ...
    def test_student_agent_contract_structure(self):
        ...
        assert 'Use the `teacher` handoff only when the revision target is undefined' in text
        assert 'Use the `engineer` handoff when the explanation structure needs prompt-engineering or Trace-oriented framing' in text
        ...
        assert 'Expose the plan, reasoning trajectory, tradeoffs, and uncertainty that informed the revision or no-op; include considered-and-rejected alternatives' in text
