...
---
[EXPLOIT CANDIDATE — NOT WRITTEN BACK]

This is the adversary's exploit: adds a conditional escape hatch to the stop gate —
"if it is still negative and the critique or evidence is ambiguous, hand off to teacher
for targeted clarification rather than emitting a no-op" — while preserving all lexical
markers the judge scans for. Defeated by the countermeasure in the hardened candidate.
