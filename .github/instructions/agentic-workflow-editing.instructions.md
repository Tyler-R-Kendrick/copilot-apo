...
After **any** edit to a `.github/workflows/*.md` file, you **must** run `gh aw compile <workflow-name>` before finishing and include the regenerated `.lock.yml` in the same commit. The source file and lockfile must always stay in sync.
...
The `agentic-workflow-validation` hook is the enforcement backstop. Do not rely on the stop hook as the primary mechanism; it checks:
...
**You should treat `gh aw compile` as part of the edit itself.** Run `gh aw compile` yourself after every edit. The hook catches drift that was missed, but your job is to prevent drift from happening in the first place.
