...
- After compiling, verify that `git diff HEAD --name-only` shows both the source `.md` and the `.lock.yml` before committing. Run `gh aw compile` one final time before opening a pull request to confirm the lockfile is in sync with all changes.
